...
def opponent_action(sensors, params, state):
    ...
    left_side  = min(opp[8:18])    # car along the left flank 
    right_side = min(opp[19:29])   # car along the right flank 
    ...
    offset_inc      = params.get("opp_offset_inc", 0.04)   

    speed = sensors.get("speedX", 0)

    # Closing speed on the car ahead 
    DT = 0.02
    ...
    if left_side < side_dist and right_side < side_dist:
        ...
        if side is None:
            if fwd_dist < slow_dist:
                #  Take the side with more room.
                track     = sensors.get("track", [100] * 19)
                imbalance = sum(track[10:19]) / 9 - sum(track[0:9]) / 9   
                if abs(imbalance) > 10:  
